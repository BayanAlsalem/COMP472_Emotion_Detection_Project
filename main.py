...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if outputs.shape[0] != labels.shape[0]:
            print(f"Error: Output and label batch sizes do not match. Outputs: {outputs.shape}, Labels: {labels.shape}")
            continue
...
best_model = CNN_VAR_3(num_classes)
